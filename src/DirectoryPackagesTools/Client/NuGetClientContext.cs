...
namespace DirectoryPackagesTools.Client
{
    ...
    public class NuGetClientContext : IDisposable
    {
        ...
        internal NuGetClientContext(SourceRepositoryProvider repos, ILogger logger, CancellationToken token, SourceCacheContext externalCache = null)
        {
            if (externalCache == null)
            {
                externalCache = _DisposableCache = NullSourceCacheContext.Instance;
            }            

            _Repos = repos;
            Logger = logger;
            _ExternalCache = externalCache ?? NullSourceCacheContext.Instance;
        ...
        internal SourceCacheContext _ExternalCache;
        private SourceCacheContext _DisposableCache;
    ...
    public class SourceRepositoryAPI
    {
        ...
        {
            if (_Context._ExternalCache == null) throw new ObjectDisposedException("Context");

            if (_APIsCache.TryGetValue(typeof(T), out var api)) return await Task.FromResult(api as T);

            api = await _Repo.GetResourceAsync<T>(_Context._Token);
        ...
        public async Task<NuGetVersion[]> GetVersionsAsync(string packageId, bool includePrerelease, bool includeUnlisted)
        {
            ...
            var vvv = await resM.GetVersions(packageId, includePrerelease, includeUnlisted, _Context._ExternalCache, _Context.Logger, _Context._Token);
        ...
        public async Task<NuGetVersion[]> GetVersionsAsync(string packageId)
        {
            ...
            var vvv = await resPID.GetAllVersionsAsync(packageId, _Context._ExternalCache, _Context.Logger, _Context._Token);
        ...
        public async Task<IPackageSearchMetadata[]> GetMetadataAsync(string packageId, bool includePrerelease = true, bool includeUnlisted = true)
        {
            ...
            var psm = await resPM.GetMetadataAsync(packageId, includePrerelease, includeUnlisted, _Context._ExternalCache, _Context.Logger, _Context._Token);
        ...
        public async Task<IPackageSearchMetadata> GetMetadataAsync(PackageIdentity package)
        {
            ...
            return await resPM.GetMetadataAsync(package, _Context._ExternalCache, _Context.Logger, _Context._Token);
        }
        ...
        public async Task<FindPackageByIdDependencyInfo> GetDependencyInfoAsync(PackageIdentity package)
        {
            ...
            return await resPID.GetDependencyInfoAsync(package.Id, package.Version, _Context._ExternalCache, _Context.Logger, _Context._Token);
        }
        ...
        public async Task<SourcePackageDependencyInfo> GetPackageDependenciesAsync(PackageIdentity package, NuGetFramework framework)
        {
            ...
            return await dependencyInfoResource.ResolvePackage(package, framework, _Context._ExternalCache, _Context.Logger, _Context._Token);
        }
